...
class MarketTiming(strategy.BacktestingStrategy):
    ...
    def _getRank(self, instrument):
        ...
        if len(priceDS) >= lookBack and smas[-1] is not None and smas[-1*lookBack] is not None:
            for i in range(-1,-1*lookBack-1,-1):
                dailyReturn += ((priceDS[i]-priceDS[i-1]) / float(priceDS[i-1]))*100
                dailyReturnList.append(dailyReturn)
            avgdailyPctReturn = (dailyReturn / float(lookBack))
            dailyReturnstddev = numpy.std(dailyReturnList)
            ret = (priceDS[-1] - priceDS[-1*lookBack]) / float(priceDS[-1*lookBack])
            #ret = avgdailyPctReturn / float(dailyReturnstddev)
        return ret
...
def main(plot):
    ...
    feed = yahoofinance.build_feed(instruments, 2000, 2015, "data/FS", skipErrors=True)
    ...
    if (returnsAnalyzer.getCumulativeReturns()[-1] * 100)/float(12) > 10:
     ...
     AvgAnnRet = (returnsAnalyzer.getCumulativeReturns()[-1] * 100)/float(12)
